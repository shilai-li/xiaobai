...
#define WELCOME_PROMPT_FIRST_ID   5000U
#define WAKEUP_PROMPT_FIRST_ID    5005U
#include "asr_api.h"
#include "user_msg_deal.h"
...
{
    /* SWM221: report Idle when playback finishes */
...
void play_exit_wakeup_done_cb(cmd_handle_t cmd_handle)
{
    ...
    /* SWM221: report Idle when exiting wakeup */
    swm221_report_state(SWM221_STATE_IDLE);
}
...
void exit_wakeup_deal(uint32_t asr_busy_check)
{
    ...
    /* 无论何种方式退出唤醒，都要强制清除在线模式标志，防止第二次唤醒时仍被误认为在线 */
...
void enter_wakeup_deal(uint32_t exit_wakup_ms, cmd_handle_t cmd_handle )
{
    ...
    #endif
    {
        ...
        #endif
        {
            ...
            #endif
            {
                ...
                if (cmd_info_get_command_id(cmd_handle) == 301U)
                {
                    prompt_play_by_voice_id(WAKEUP_PROMPT_FIRST_ID,
                                            play_enter_wakeup_done_cb, true);
                }
                else
                {
            ...
            else
            {
                ...
                {
                    prompt_play_by_voice_id(WAKEUP_PROMPT_FIRST_ID,
                                            default_play_done_callback, true);
                }
                else
                {
                    prompt_play_by_cmd_handle(cmd_handle, -1,
                                              default_play_done_callback, true);
                }
                #else
    ...
    /* SWM221: report Listening when wake-up is entered */
    swm221_report_state(SWM221_STATE_LISTENING);
    swm221_send_packet(cmd_info_get_command_id(cmd_handle));
...
{
    /* Keep the wakeup window alive until cloud audio is audibly finished. */
    if (gCiasAiotRunParam.cloud_play_state != CLOUD_PLAY_END || check_current_playing())
    {
        update_awake_time();
        return;
    }
...
void sys_deal_asr_msg(sys_msg_asr_data_t *asr_msg)
{
    ...
    if((MSG_ASR_STATUS_GOOD_RESULT == asr_msg->asr_status||(MSG_CWSL_STATUS_GOOD_RESULT == asr_msg->asr_status)) && (ignore_asr_msg == 0))
    {
        ...
        else if (SYS_STATE_WAKEUP == get_wakeup_state())
        {
            ...
            if (cmd_id != 317)
            {
                cias_send_cmd_and_data(LOCAL_ASR_RESULT_NOTIFY, cmd_id_buf, 2, DEF_FILL);
            }
            //重置录音参数
            gCiasAiotRunParam.is_vad_on_flag = false;
            ...
            switch (rev_msg.msg_type)
            {
                ...
                case SYS_MSG_TYPE_AUDIO_IN_STARTED:
                {
                    ...
                    #endif
                    {
                        #if PLAY_WELCOME_EN
                        /*
                         * 延迟开机提示音，让 ESP32 有时间完成 4G 入网和激活。
                         * ESP32 联网提示音通常会在此之前播放完成。
                         */
                        vTaskDelay(pdMS_TO_TICKS(WELCOME_PROMPT_DELAY_MS));
                        ...
                        {
                            prompt_play_by_voice_id(WELCOME_PROMPT_FIRST_ID, NULL, true);
                        }
                        else
                        {
