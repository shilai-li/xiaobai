...
#define LCD_GPIO_CS2           GPIOA
#define LCD_PIN_CS2            PIN11
...
static uint8_t spi_dma_xfer(void *data, uint32_t pixels, uint8_t async, uint8_t src_inc)
{
    ...
    GPIO_AtomicClrBit(LCD_GPIO_CS, LCD_PIN_CS);
	   GPIO_AtomicClrBit(LCD_GPIO_CS2, LCD_PIN_CS2);
    ...
    if (0 == async) {
        ...
        GPIO_AtomicSetBit(LCD_GPIO_CS, LCD_PIN_CS);
			GPIO_AtomicSetBit(LCD_GPIO_CS2, LCD_PIN_CS2);
        /* 释放 DMA 写 SPI-FIFO */
        LCD_SPI_X->CTRL &= ~SPI_CTRL_DMATXEN_Msk;
    }
    return 0;
...
void __DECL_LCD_NAME(port_init, spi_9)(void)
{
    ...
    GPIO_INIT(LCD_GPIO_CS2, LCD_PIN_CS2, GPIO_OUTPUT);
    GPIO_AtomicSetBit(LCD_GPIO_CS2, LCD_PIN_CS2);
...
void __DECL_LCD_NAME(port_deinit, spi_9)(void)
{
    ...
    GPIO_AtomicSetBit(LCD_GPIO_CS, LCD_PIN_CS);
	    ...
	    GPIO_INIT(LCD_GPIO_CS2, LCD_PIN_CS2, GPIO_OUTPUT); // 片选必须拉高
    GPIO_AtomicSetBit(LCD_GPIO_CS2, LCD_PIN_CS2);
...
{
	//printf("flush_bitmap_async_done\r\n");
