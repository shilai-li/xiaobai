...
public:
    ...
    // Audio already handed to the codec but not yet played. Codecs that buffer internally report it
    // so the playback pipeline can measure its true depth instead of only the upstream queues.
    virtual int OutputBufferedMs() const { return 0; }
    // The application signalled start/end of a TTS stream. Codecs that batch a whole response
    ...
    virtual bool PlaybackCompletionIsAsync() const { return false; }
