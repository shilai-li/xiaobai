...
void AudioService::Initialize(AudioCodec* codec) {
    ...
    audio_processor_->OnOutput([this](std::vector<int16_t>&& data) {
        const int64_t afe_output_time_us = esp_timer_get_time();
        const int64_t mic_capture_time_us = ConsumeMicPcmTimestamp(data.size());
        PushTaskToEncodeQueue(kAudioTaskTypeEncodeToSendQueue, std::move(data),
                              mic_capture_time_us, afe_output_time_us);
    });
...
bool AudioService::ReadAudioData(std::vector<int16_t>& data, int sample_rate, int samples) {
    ...
    const int64_t ci130x_read_start_us = esp_timer_get_time();
    ...
    const int64_t ci130x_read_end_us = esp_timer_get_time();
    static uint32_t ci130x_read_seq = 0;
    if (ci130x_read_seq++ % 100 == 0) {
        ESP_LOGI(TAG, "Mic RX seq=%u samples=%u ci130x_to_esp=%dms",
                 static_cast<unsigned>(ci130x_read_seq - 1), static_cast<unsigned>(data.size()),
                 static_cast<int>((ci130x_read_end_us - ci130x_read_start_us) / 1000));
    }
        ...
        if (bits & (AS_EVENT_WAKE_WORD_RUNNING | AS_EVENT_AUDIO_PROCESSOR_RUNNING)) {
            ...
            if (ReadAudioData(data, 16000, samples)) {
                ...
                if (bits & AS_EVENT_AUDIO_PROCESSOR_RUNNING) {
                    RecordMicPcmTimestamp(esp_timer_get_time());
...
void AudioService::AudioOutputTask() {
    while (true) {
        ...
        audio_queue_cv_.wait(lock, [this]() {
            ...
            if (!streaming_playback_active_ || streaming_playback_ready_) {
                return true;
            }

            const size_t buffered_ms = BufferedPlaybackDurationMsLocked();
        ...
        const int64_t ci130x_write_start_us = esp_timer_get_time();
        codec_->OutputData(task->pcm);
        const int64_t ci130x_write_end_us = esp_timer_get_time();
        static uint32_t playback_seq = 0;
        if (playback_seq++ % 50 == 0) {
            const int rx_to_ci_ms = task->esp_timestamp_us == 0
                ? -1
                : static_cast<int>((ci130x_write_start_us - task->esp_timestamp_us) / 1000);
            ESP_LOGI(TAG, "TTS TX seq=%u samples=%u ml307_to_ci=%dms ci_write=%dms",
                     static_cast<unsigned>(playback_seq - 1), static_cast<unsigned>(task->pcm.size()), rx_to_ci_ms,
                     static_cast<int>((ci130x_write_end_us - ci130x_write_start_us) / 1000));
        }
        ...
        // Once startup buffering has completed, keep feeding the codec continuously.
        // Re-entering a second buffering phase on a transient queue low-water mark pauses the
        // proven CI130X PLAY_GET flow and can itself create an audible underrun.
        lock.lock();
        ...
        if (streaming_playback_active_) {
            if (streaming_playback_finished_ &&
                ...
                streaming_playback_ready_ = false;
        ...
        // Only now has the last frame of the response reached the codec. Telling it earlier (when
        // the server stopped sending) would let it end the session while our queues still held
        // seconds of audio it had not been given yet.
        if (pipeline_drained) {
    ...
    while (true) {
        ...
        audio_queue_cv_.wait(lock, [this]() {
            return service_stopped_ ||
                (!audio_encode_queue_.empty() && audio_send_queue_.size() < kMaxSendPacketsInQueue) ||
        ...
        if (!audio_decode_queue_.empty() && audio_playback_queue_.size() < MAX_PLAYBACK_TASKS_IN_QUEUE) {
            ...
            task->esp_timestamp_us = packet->esp_timestamp_us;
        ...
        if (!audio_encode_queue_.empty() && audio_send_queue_.size() < kMaxSendPacketsInQueue) {
            audio_encode_busy_ = true;
            ...
            packet->esp_timestamp_us = task->esp_timestamp_us;
            packet->mic_capture_time_us = task->mic_capture_time_us;
            ...
            if (opus_encoder_ != nullptr && task->pcm.size() == encoder_frame_size_) {
                ...
                packet->opus_encode_start_time_us = esp_timer_get_time();
                auto ret = esp_opus_enc_process(opus_encoder_, &in, &out);
                if (ret == ESP_AUDIO_ERR_OK) {
                    const int64_t opus_ready_time_us = esp_timer_get_time();
                    packet->payload.assign(buf.data(), buf.data() + out.encoded_bytes);
                    packet->opus_ready_time_us = opus_ready_time_us;
                    packet->opus_encode_time_us = opus_ready_time_us - packet->opus_encode_start_time_us;
...
void AudioService::WaitForUplinkEncodeIdle() {
    std::unique_lock<std::mutex> lock(audio_queue_mutex_);
    audio_queue_cv_.wait(lock, [this]() {
        return service_stopped_ || (audio_encode_queue_.empty() && !audio_encode_busy_);
    });
}
...
void AudioService::PushTaskToEncodeQueue(AudioTaskType type, std::vector<int16_t>&& pcm,
                                         int64_t mic_capture_time_us, int64_t afe_output_time_us) {
    auto task = std::make_unique<AudioTask>();
    task->type = type;
    task->pcm = std::move(pcm);
    task->esp_timestamp_us = afe_output_time_us != 0 ? afe_output_time_us : esp_timer_get_time();
    task->mic_capture_time_us = mic_capture_time_us;
...
void AudioService::RecordMicPcmTimestamp(int64_t timestamp_us) {
    std::lock_guard<std::mutex> lock(mic_timestamp_mutex_);
    // The AFE produces 60 ms frames from 10 ms PCM reads. Retain a little
    // extra history for scheduling jitter, then drop stale timestamps safely.
    if (mic_pcm_timestamps_.size() >= 64) {
        mic_pcm_timestamps_.pop_front();
    }
    mic_pcm_timestamps_.push_back(timestamp_us);
}

int64_t AudioService::ConsumeMicPcmTimestamp(size_t output_samples) {
    constexpr size_t kInputChunkSamples = 160;  // 10 ms at 16 kHz mono
    const size_t input_chunks = std::max<size_t>(1, (output_samples + kInputChunkSamples - 1) / kInputChunkSamples);

    std::lock_guard<std::mutex> lock(mic_timestamp_mutex_);
    if (mic_pcm_timestamps_.empty()) {
        return 0;
    }
    const int64_t first_timestamp_us = mic_pcm_timestamps_.front();
    const size_t chunks_to_remove = std::min(input_chunks, mic_pcm_timestamps_.size());
    for (size_t i = 0; i < chunks_to_remove; ++i) {
        mic_pcm_timestamps_.pop_front();
    }
    return first_timestamp_us;
}

bool AudioService::PushPacketToDecodeQueue(std::unique_ptr<AudioStreamPacket> packet, bool wait) {
    std::unique_lock<std::mutex> lock(audio_queue_mutex_);
    // Keep the queue limit duration-based so changing the negotiated Opus frame
    // duration does not silently change the amount of buffered audio.
    const int frame_duration =
        packet != nullptr && packet->frame_duration >= 20
            ? packet->frame_duration
            : OPUS_FRAME_DURATION_MS;
    const size_t duration_based_limit =
        std::max<size_t>(1, MAX_DECODE_BUFFER_DURATION_MS / frame_duration);
    const size_t max_decode_packets = duration_based_limit;
    if (audio_decode_queue_.size() >= max_decode_packets) {
        if (wait) {
            audio_queue_cv_.wait(lock, [this, max_decode_packets]() {
                return audio_decode_queue_.size() < max_decode_packets;
            });
        } else {
            return false;
        }
...
void AudioService::BeginStreamingPlayback(size_t prebuffer_ms) {
    ResetDecoder();
    codec_->NotifyOutputStreamStart();

    std::lock_guard<std::mutex> lock(audio_queue_mutex_);
    streaming_playback_active_ = true;
    streaming_playback_ready_ = false;
    streaming_playback_finished_ = false;
    // A prebuffer deeper than the decode queue can hold would never be reached, so playback would
    // only start once the response ended.
    streaming_prebuffer_ms_ = std::min<size_t>(prebuffer_ms, MAX_DECODE_BUFFER_DURATION_MS);
    ESP_LOGI(TAG, "Streaming playback buffering started: target=%u ms, max=%d ms",
             static_cast<unsigned>(streaming_prebuffer_ms_),
             MAX_DECODE_BUFFER_DURATION_MS);
}

void AudioService::FinishStreamingPlayback() {
    std::unique_lock<std::mutex> lock(audio_queue_mutex_);
    // Already empty means the codec has everything it is ever going to get, and the output task
    // will not pop again to notice that itself.
    const bool drained = !streaming_playback_active_ ||
                         (audio_decode_queue_.empty() &&
                          audio_playback_queue_.empty() &&
                          !audio_output_busy_);
    if (streaming_playback_active_) {
        streaming_playback_finished_ = true;
        if (drained) {
            streaming_playback_active_ = false;
            streaming_playback_ready_ = false;
        }
        // Release a short final segment even if it never reached the normal
        // startup threshold.
        audio_queue_cv_.notify_all();
    }
    lock.unlock();
...
size_t AudioService::BufferedPlaybackDurationMs() {
    std::lock_guard<std::mutex> lock(audio_queue_mutex_);
    return BufferedPlaybackDurationMsLocked();
}

size_t AudioService::BufferedPlaybackDurationMsLocked() const {
    // Audio already handed to the codec is still waiting to be heard, so it counts. Without it
    // the queues look empty the instant they drain into the codec's own buffer.
    size_t duration_ms = static_cast<size_t>(codec_->OutputBufferedMs());
    duration_ms += audio_playback_queue_.size() * static_cast<size_t>(decoder_duration_ms_);
    for (const auto& packet : audio_decode_queue_) {
        const int frame_duration =
            packet != nullptr && packet->frame_duration > 0
                ? packet->frame_duration
                : decoder_duration_ms_;
        duration_ms += static_cast<size_t>(frame_duration);
    }
    return duration_ms;
}
...
void AudioService::EnableVoiceProcessing(bool enable) {
    // Turn boundary: kept at INFO so the default log level still shows where a
    // listening turn starts and stops.
