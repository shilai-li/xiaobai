...
// Must hold the startup prebuffer plus however far ahead of real time the server runs over a whole
// response; at 2000 ms a long reply overflowed and dropped audio. This is also the real jitter
// ceiling, because the queue keeps filling past the prebuffer threshold: a 4830 ms cellular stall
...
// CI130X local-command routing holds a complete VAD segment before deciding
// whether it may leave the device. Include rollback and encoder headroom.
constexpr size_t kMaxSendPacketsInQueue =
    ((kMaximumVadStartTimeoutSeconds * 1000 + kCi130xHeldAudioMarginMs) +
     OPUS_FRAME_DURATION_MS - 1) / OPUS_FRAME_DURATION_MS;
static_assert(kMaxSendPacketsInQueue * OPUS_FRAME_DURATION_MS >=
                  kMaximumVadStartTimeoutSeconds * 1000 + kCi130xHeldAudioMarginMs,
              "Send queue cannot hold the maximum gated VAD segment");
#else
constexpr size_t kMaxSendPacketsInQueue = 2400 / OPUS_FRAME_DURATION_MS;
#endif
...
struct AudioTask {
    ...
    uint32_t timestamp;
    int64_t esp_timestamp_us = 0;
    int64_t mic_capture_time_us = 0;
};
...
public:
    ...
    void BeginStreamingPlayback(size_t prebuffer_ms = STREAM_PLAYBACK_PREBUFFER_DURATION_MS);
    void FinishStreamingPlayback();
    size_t BufferedPlaybackDurationMs();
...
private:
    ...
    std::mutex mic_timestamp_mutex_;
    std::deque<int64_t> mic_pcm_timestamps_;
    // For server AEC
    ...
    bool streaming_playback_ready_ = false;
    bool streaming_playback_finished_ = false;
    size_t streaming_prebuffer_ms_ = STREAM_PLAYBACK_PREBUFFER_DURATION_MS;
    bool audio_output_busy_ = false;
    bool audio_encode_busy_ = false;
    ...
    void PushTaskToEncodeQueue(AudioTaskType type, std::vector<int16_t>&& pcm,
                               int64_t mic_capture_time_us = 0, int64_t afe_output_time_us = 0);
    void RecordMicPcmTimestamp(int64_t timestamp_us);
    int64_t ConsumeMicPcmTimestamp(size_t output_samples);
    void SetDecodeSampleRate(int sample_rate, int frame_duration);
    void CheckAndUpdateAudioPowerState();
    size_t BufferedPlaybackDurationMsLocked() const;
};
