...
public:
    ...
    virtual bool SyncSystemTime() { return false; }
    ...
    // Returns false on boards without a cellular modem or when the serving-cell
    // information is unavailable.
    virtual bool GetServingCellInfo(ServingCellInfo& info) { (void)info; return false; }
    // Boards whose voice front-end stays awake between turns can keep the conversation socket
    // open on idle, so the next turn does not pay a handshake while the user is already talking.
    virtual bool KeepAudioChannelOnIdle() { return false; }
};
