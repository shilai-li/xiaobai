...
bool WifiBoard::SyncSystemTime() {
    // Already synchronized (e.g. by a previous call)?
    time_t now;
    time(&now);
    struct tm timeinfo;
    ...
    ESP_LOGI(TAG, "Synchronizing system time via SNTP...");
    ...
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "Failed to synchronize system time via SNTP: %s", esp_err_to_name(err));
        return false;
    }

    time(&now);
    localtime_r(&now, &timeinfo);
    ESP_LOGI(TAG, "System time synchronized: %04d-%02d-%02d %02d:%02d:%02d UTC",
