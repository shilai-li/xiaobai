...
bool DeviceStateMachine::IsValidTransition(DeviceState from, DeviceState to) const {
    ...
    // Revocation is a terminal business state and may interrupt any normal
    // operating state immediately. Fatal errors remain terminal on their own.
    if (to == kDeviceStateRevoked && from != kDeviceStateFatalError) {
        return true;
    }
    ...
    switch (from) {
        ...
        case kDeviceStateConnecting:
            // Can go to idle (failed/finished), listening (connected), or
            // speaking (server response after vad_done or topic TTS).
            return to == kDeviceStateIdle ||
                   to == kDeviceStateListening ||
                   to == kDeviceStateSpeaking;

        case kDeviceStateListening:
            // After vad_done, connecting represents waiting for the server reply.
        ...
        case kDeviceStateRevoked:
            // Credentials must be provisioned again outside the normal runtime
            // before a revoked device can become usable.
            return false;
