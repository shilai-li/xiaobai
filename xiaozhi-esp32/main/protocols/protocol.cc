...
void Protocol::SendCancelTurn(const std::string& reason) {
    std::string message = "{\"session_id\":\"" + session_id_ + "\",\"type\":\"cancel_turn\",\"reason\":\"" + reason + "\"}";
    SendText(message);
}
...
bool Protocol::SendTts(const std::string& id, const std::string& text) {
    cJSON* root = cJSON_CreateObject();
    cJSON_AddStringToObject(root, "type", "tts");
    cJSON* payload = cJSON_CreateObject();
    cJSON_AddStringToObject(payload, "id", id.c_str());
    cJSON_AddStringToObject(payload, "text", text.c_str());
    cJSON_AddItemToObject(root, "payload", payload);
    char* json = cJSON_PrintUnformatted(root);
    const bool sent = SendText(json);
    cJSON_free(json);
    cJSON_Delete(root);
    return sent;
}
